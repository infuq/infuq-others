...
# 特别行政区: 香港 澳门
def is_special_province(province):
    return province == '81' or province == '82'
...
def query(url):
    ...
    html_tree = etree.HTML(response.text.lower())
    all_tr = html_tree.xpath('//table/tbody/tr')
    # 所有tr
    for tr in all_tr:
        # 单个tr下所有td的文本
        ...
        if pair is None:
            ...
            code = pair[0]
            name_arr = tr.xpath('*')[2].xpath('.//font/text()')
            for _name in name_arr:
                name = name + _name


        if code is None or name is None or len(name) < 1:
            continue
...
def handle_province(code, name, cursor, conn):
    ...
    if is_direct_province(province):  # 直辖市
        ...
        cursor.execute(sql, (short_code, code, name, name, level, year, parent_id, desc))
        conn.commit()
    elif is_special_province(province):
        desc = "特别行政区"
...
if __name__ == '__main__':
    year = 2016
    url = 'https://www.mca.gov.cn/mzsj/xzqh/1980/201705/201705311652.html'
    handle(url)
