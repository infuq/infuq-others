    ...
    <section class="content clearfix" style="width: 97%;">
      ...
      <el-form :inline="true" class="demo-form-inline">
        ...
        <div>
          ...
          <el-input v-model="goodsName" placeholder="商品名称" clearable @clear="clearGoodsName"
                    style="width: 400px; margin-left: 5px;">
            <template slot="prepend">商品名称</template>
          </el-input>

          <el-input v-model="goodsOwnerName" placeholder="货主" clearable @clear="clearGoodsOwnerName"
        ...
        <div>
          <el-input v-model="warehouse_owner_goods_code" placeholder="STOCK20220401175506943hpQU" clearable
                    @clear="clear_warehouse_owner_goods_code" style="width: 400px;">
        ...
        <div>
          <el-input v-model="business_no" placeholder="NULL-0001-000032 或 388404498978394112" clearable
                    @clear="clear_business_no" style="width: 600px;">
            ...
            <div style="font-weight: 500; font-size: 14px;">
                ...
                <el-table :data="swogList" border style="width: 100%" :row-style="{ height: 10+'px'}" :cell-style="{padding:0+'px'}"> <!-- height="250" -->
                      ...
                      <el-form label-position="left" inline class="demo-table-expand">
                        ...
                        <div style="margin-left: 40px; font-weight: 500; font-size: 14px;">
                          <div style="width: 400px; display: block; margin: 5px 0;">
                            <label style="display: inline-block; width: 130px; text-align: right;">历史商品名称:</label><label>&nbsp;&nbsp;<el-button
                              @click="handleGoodsNameHistoryClick(props.row)" type="text">查看
                          </el-button>
                          </label>
                          </div>
                        </div>
                        <div style="margin-left: 40px; font-weight: 500; font-size: 14px;">
                          <div style="width: 400px; display: block; margin: 5px 0;">
                            <label
                                style="display: inline-block; width: 130px; text-align: right;">OMS商品编码:</label><label>&nbsp;&nbsp;&nbsp;{{ props.row.externalGoodsNo }}&nbsp;&nbsp;<span
                              v-if="props.row.externalGoodsNo != null && props.row.externalGoodsNo != ''"><i
                              @click="copy(props.row.externalGoodsNo)" class="el-icon-document-copy"></i></span></label>
                          </div>
                        </div>
                        <div style="margin-left: 40px; font-weight: 500; font-size: 14px;">
                          <div style="width: 400px; display: block; margin: 5px 0;">
                            <label
                                style="display: inline-block; width: 130px; text-align: right;">仓库货主商品编码:</label><label>&nbsp;&nbsp;&nbsp;{{ props.row.warehouseOwnerGoodsNo }}&nbsp;&nbsp;<i
                              @click="copy(props.row.warehouseOwnerGoodsNo)" class="el-icon-document-copy"></i></label>
                          </div>
                        </div>
                        <div style="margin-left: 40px; font-weight: 500; font-size: 14px;">
                          <div style="width: 400px; display: block; margin: 5px 0;">
                        ...
                        <div style="margin-left: 40px; font-weight: 500; font-size: 14px;">
                          <div style="width: 400px; display: block; margin: 5px 0;">
                            <label style="display: inline-block; width: 130px; text-align: right;">货主编码:</label><label>&nbsp;&nbsp;&nbsp;{{ props.row.customerNo }}&nbsp;&nbsp;<i
                              @click="copy(props.row.customerNo)" class="el-icon-document-copy"></i></label>
                          </div>
                        </div>
                  ...
                  <el-table-column fixed label="商品名称" min-width="150" :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                      ...
                      <el-button @click="handleGoodsBatchHistoryClick(scope.row)" type="text">{{scope.row.goodsName }}
                      </el-button>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column prop="availableQuantity" width="110">
                    <template slot-scope="scope" slot="header">
                                  <span>
                                    可用库存
                                    <el-tooltip :aa="scope" class="item" effect="dark"
                                                content="store_batch_details#availableQuantity" placement="top-start">
                  ...
                  <el-table-column width="110">
                    <template slot-scope="scope" slot="header">
                                  <span>
                                    冻结库存
                                    <el-tooltip :aa="scope" class="item" effect="dark"
                                                content="store_batch_details#outFrozenQuantity" placement="top-start">
                                        <i class='el-icon-warning' style="color:#409eff; margin-left:5px;'"> </i>
                                    </el-tooltip>
                                  </span>
                    </template>
                    ...
                    <template slot-scope="scope">
                                  <span v-if="scope.row.outFrozenQuantity > 0">
                                      <el-button @click="handleFrozenQuantityClick(scope.row)"
                                                 type="text">{{ scope.row.outFrozenQuantity }}</el-button>
                                  </span>
                      <span v-else>
                  ...
                  <el-table-column prop="availableSaleCount">
                    <template slot-scope="scope" slot="header">
                                  <span>
                                    可售库存
                                    <el-tooltip :aa="scope" class="item" effect="dark"
                                                content="库存数量-待出库数量-已售库存-过期库存-待调拨数量"
                                                placement="top-start">
                                        <i class='el-icon-warning' style="color:#409eff; margin-left:5px;'"> </i>
                                    </el-tooltip>
                                  </span>
                                      ...
                                      <span style="display: inline-block; width: 150px;">
                                          <span v-if="scope.row.soldQuantity > 0 || scope.row.noAllocateCount > 0">
                                              <span v-if="scope.row.availableSaleCount < 0">
                                                  <el-button @click="handleSoldQuantityClick(scope.row)"
                                                             type="text"><span style="color: red;">{{
                                                      scope.row.availableSaleCount
                                                    }}</span></el-button>
                                              </span>
                                              <span v-else>
                                                  <el-button @click="handleSoldQuantityClick(scope.row)"
                                                             type="text">{{ scope.row.availableSaleCount }}</el-button>
                                              </span>
                                          </span>
              ...
              <el-dialog :title="'[' + DETAIL_INDEX + '][ ' + GoodsName_Batch + ' ] 库存操作记录'" :visible.sync="GoodsBatchClick_dialogVisible" :show-close="false" width="80%">
                <el-select v-model="select_batch" placeholder="请选择批次" @change="selectBatch()" filterable>
                  <el-option v-for="item in BatchOptions" :key="item.value" :label="item.label" :value="item.value">
                    ...
                    <span style="float: right; color: #8492a6; font-size: 13px">{{ item.sumAvailableQuantity }}</span>
                  </el-option>
                </el-select>
                <el-select v-model="select_batch_detail" placeholder="请选择库位" @change="selectBatchDetail(1)" filterable style="margin-left: 3px;">
                  <el-option v-for="item in BatchDetailOptions" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </el-select>
                <span style="display: inline; margin-left: 25px;" v-if="GoodsBatchDetailHistoryTotal > 0">
                  <span>共计{{ GoodsBatchDetailHistoryTotal }}条记录</span>&nbsp;&nbsp;<i @click="copy(GoodsBatchDetailHistorySQL)" class="el-icon-document-copy"></i>SQL
                </span>
                ...
                <div style="margin-top: 10px;" v-loading="goods_batch_loading">
                  <template>
                    ...
                    <el-table :data="GoodsBatchDetailHistoryList" border style="width: 100%" :row-style="{ height: 10+'px'}" :cell-style="{padding:0+'px'}" v-loading="LoadingGoodsBatchDetailHistoryList">
                      ...
                      <el-table-column align="center" label="出入库单号" width="180">
                        <template slot-scope="scope">
                          <span v-if="scope.row.storageNo !== '' && scope.row.storageNo.startsWith('CK')">
                            <router-link :to="{path: '/store_storage', query:{storeStorageNo: scope.row.storageNo}}" target="_blank" style="margin-left: 5px;">
                              <span class="icon-link">{{ scope.row.storageNo }}</span>
                            </router-link>
                          </span>
                          <span v-else-if="scope.row.storageNo !== '' && scope.row.storageNo.startsWith('RK')">
                            <router-link :to="{path: '/store_storage', query:{storeStorageNo: scope.row.storageNo}}" target="_blank" style="margin-left: 5px;">
                              <span class="icon-link">{{ scope.row.storageNo }}</span>
                            </router-link>
                          </span>
                      ...
                      <el-table-column align="center" label="源单号" width="185">
                        <template slot-scope="scope">
                          <span v-if="scope.row.sourceOrderNo != undefined && scope.row.sourceOrderNo != '' && scope.row.sourceOrderNo.startsWith('DH')">
                            <router-link :to="{path: '/store_customer_order', query:{customerOrderNo: scope.row.sourceOrderNo}}" target="_blank" style="margin-left: 5px;">
                              <span class="icon-link">{{ scope.row.sourceOrderNo }}</span>
                            </router-link>
                          </span>
                          <span v-else-if="scope.row.sourceOrderNo != undefined && scope.row.sourceOrderNo != '' && scope.row.sourceOrderNo.startsWith('DB')">
                            <router-link :to="{path: '/allocate_order', query:{allocateOrderNo: scope.row.sourceOrderNo}}" target="_blank" style="margin-left: 5px;">
                              <span class="icon-link">{{ scope.row.sourceOrderNo }}</span>
                            </router-link>
                          </span>
                          <span v-else>
              ...
              <el-dialog :title="'[' + DETAIL_INDEX + '][ ' + GoodsName_Batch + ' ] 商品名称历史记录'" :visible.sync="GoodsNameClick_dialogVisible" :show-close="false" width="80%">
                ...
                <div style="margin-top: 10px;">
                  <template>
                    <el-table :data="nameHistoryList" border style="width: 100%" :row-style="{ height: 10+'px'}" :cell-style="{padding:0+'px'}">
                      <el-table-column fixed label="序号" type="index" width="50">
                        <template slot-scope="scope">
                          {{ scope.$index + 1 }}
                        </template>
              ...
              <el-dialog :title="'[' + DETAIL_INDEX + '][ ' + GoodsName_Batch + ' ] 库存冻结记录'" :visible.sync="GoodsFrozenClick_dialogVisible" :show-close="false" width="75%">
                ...
                <div style="margin-top: 10px;" v-loading="goods_frozen_loading">
                  <template>
                    <el-table :data="frozenList" border style="width: 100%" :row-style="{ height: 10+'px'}" :cell-style="{padding:0+'px'}">
                      <el-table-column fixed label="序号" type="index" width="50">
                        <template slot-scope="scope">
                          {{ scope.$index + 1 }}
                        </template>
                      ...
                      <el-table-column align="center" label="出库单号" width="180">
                        <template slot-scope="scope">
                          <el-link type="primary" :href="'store_storage.html?storeStorageNo='+scope.row.storageNo"
                                   :underline="false" target="_blank">{{ scope.row.storageNo }}
                          </el-link>
                        </template>
                      </el-table-column>
                      <el-table-column align="center" prop="sourceOrderNo" label="源单号" width="180"></el-table-column>
                      <el-table-column align="center" prop="expectQuantity" label="冻结数量"
              ...
              <el-dialog :title="'[' + DETAIL_INDEX + '][ ' + GoodsName_Batch + ' ] 已售库存和待调拨数量'"
                         :visible.sync="GoodsSoldClick_dialogVisible" :show-close="false" width="50%">

                <div style="margin-top: 10px;">
                  <template>
                    <el-table :data="soldOrderList" border style="width: 100%" :row-style="{ height: 10+'px'}"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      LoadingGoodsBatchDetailHistoryList: false,
  ...
  methods: {
    ...
    onSubmit(curPage) {
      ...
      this.swogList = []
      let _warehouseCode = this.select_warehouse
      let _goodsName = this.goodsName
      let _goodsOwnerName = this.goodsOwnerName
      let _warehouse_owner_goods_code = this.warehouse_owner_goods_code
      let _business_no = this.business_no
      if (this.on_Submit === -1) {
      ...
      this.loading = true
      axios({
        url: this.host + '/toolkit/tools/getGoodsStockHistory',
        method: 'GET',
        params: {
    ...
    onSubmit2() {
      ...
      axios({
        url: this.host + '/toolkit/tools/fixStockByWarehouseOwnerGoods',
        method: 'GET',
        params: {
    ...
    handleGoodsBatchHistoryClick(row) {
      ...
      this.LoadingGoodsBatchDetailHistoryList = true
      ...
      .then(response => {
        ...
        if (batchList.length > 0) {
          for (let i = 0; i < batchList.length; i++) {
            ...
            let _sumAvailableQuantity = batchList[i]['sumAvailableQuantity']
            this.BatchOptions.push({'label': _label, 'value': _value, 'sumAvailableQuantity': _sumAvailableQuantity})
          }

          this.LoadingGoodsBatchDetailHistoryList = false
    ...
    handleGoodsNameHistoryClick(row) {
      ...
      for (let i = 0; i < row.nameHistoryList.length; i++) {
        let goodsName = row.nameHistoryList[i]['goodsName']
        let goodsUnit = row.nameHistoryList[i]['goodsUnit']
        let goodsSpec = row.nameHistoryList[i]['goodsSpec']
        let version = row.nameHistoryList[i]['version']
        let createTimeStr = row.nameHistoryList[i]['createTimeStr']
        this.nameHistoryList.push({
          'goodsName': goodsName,
          'goodsUnit': goodsUnit,
          'goodsSpec': goodsSpec,
          'version': version,
    ...
    handleFrozenQuantityClick(row) {
      ...
      axios({
        url: this.host + '/toolkit/tools/getFrozenStoreStorageByWarehouseOwnerGoodsId',
        method: 'GET',
        params: {
          warehouseOwnerGoodsId: row.warehouseOwnerGoodsId
        },
    ...
    handleSoldQuantityClick(row) {
      ...
      // 已售库存
      let soldScoList = row.soldScoList
      // 待调拨数量
      let noAllocateList = row.noAllocateList

      this.soldOrderList = [];
      if (soldScoList) {
        for (let i = 0; i < soldScoList.length; i++) {
          let customerOrderNo = soldScoList[i]['customerOrderNo']
          let soldQuantity = soldScoList[i]['soldQuantity']
          this.soldOrderList.push({'type': '已售库存', 'orderNo': customerOrderNo, 'quantity': soldQuantity})
        }
      }

      if (noAllocateList) {
        for (let i = 0; i < noAllocateList.length; i++) {
          let allocateOrderNo = noAllocateList[i]['allocateOrderNo']
          let quantity = noAllocateList[i]['quantity']
    ...
    syncStockToOms(row) {
      this.$confirm('确定同步库存到OMS?', '[ ' + row.goodsName + ' ]', {
        ...
        axios({
          url: this.host + '/toolkit/tools/syncStockToOms',
          method: 'GET',
          params: {
            warehouseOwnerGoodsId: row.warehouseOwnerGoodsId,
          },
