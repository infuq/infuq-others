            ...
            <div style="font-weight: 500; font-size: 14px;">
              <template>
                <el-table :data="receiveList" border style="width: 100%" :row-style="{ height: 10+'px'}"
                  ...
                  <el-table-column sortable prop="warehouseName" label="仓库名称" max-width="220"
                  ...
                  <el-table-column prop="receiveApplicationName" label="来源平台" width="100"
                                   :show-overflow-tooltip="true"></el-table-column>
                  <el-table-column prop="receiveTypeName" label="接收类型" width="200"
                                   :show-overflow-tooltip="true"></el-table-column>
                  <el-table-column sortable :sort-method="sortReceiveStatus" label="接收状态" width="120">
                    <template slot-scope="scope">
                                  <span v-if="scope.row.receiveStatusName == '处理失败'">
                                      <span style="color: red;">{{ scope.row.receiveStatusName }}</span>
                                  </span>
                      <span
                          v-else-if="scope.row.receiveStatusName == '处理成功' || scope.row.receiveStatusName == '已完成'">
                  ...
                  <el-table-column fixed="right" label="操作">
                    <template slot-scope="scope">
                      ...
                      <span v-if="scope.row.receiveStatusName == '处理失败' || scope.row.receiveStatusName == '待处理'">
              ...
              <el-dialog :title="'[ ' + Record_Body_Title + ' ] 内容体'" :visible.sync="Record_Body_dialogVisible"
                ...
                <div style="display: block; margin: 5px 0;">
                  <label style="display: inline-block; text-align: right;">[ 处理内容 ]</label><label>&nbsp;&nbsp;&nbsp;{{ receiveRecord.actionBody }}</label>
                </div>
                <div style="display: block; margin: 5px 0;">
                  <label style="display: inline-block; text-align: right;">[ 完成内容 ]</label><label>&nbsp;&nbsp;&nbsp;{{ receiveRecord.finishBody }}</label>
                </div>
                <div style="display: block; margin: 5px 0;">
                  <label style="display: inline-block; text-align: right;">[ 回执内容 ]</label><label>&nbsp;&nbsp;&nbsp;{{ receiveRecord.receiptBody }}</label>
                </div>
              </el-dialog>
            </div>
          </div>
...
export default {
  ...
  created: function () {
    ...
    if (customerOrderCode) {
      ...
      let t = new Date(time)
      // 该时间点   OMS的单据已经通过接口平台发送给WMS了
      let d = new Date('2024-03-13 06:30:00')
      if (t.getTime() < d.getTime()) {
        this.thirdNo = customerOrderCode
      } else {
        this.businessNo = customerOrderCode
      }
      this.receiveBeginTime = time.substr(0, 10) + " 00:00:00"
      this.receiveEndTime = time.substr(0, 10) + " 23:59:59"
      this.onSubmit()
    }
  ...
  methods: {
    ...
    onSubmit(curPage) {
      ...
      this.loading = true
      axios({
        url: this.host + '/toolkit/tools/getReceiveRecord',
        method: 'POST',
        data: {
